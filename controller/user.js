...
let { registerUser, loginUser, forgetUser, resetPassword, getMe, updateMe, changePass,addprofile, deactivate,updateProfile, findAll, assignPer, getpermission, getallpermission, update, softDelete, softUndelete, active, unActive  } = require("../model/user")
...
async function update_profile(request, response) {
    let file = await uploads(request, response, "update_pic", { destination: './user-images/' ,
     fileSize: 3 * 3000 * 300 }).catch((err) => {
        return { error: err }
    });
    if (!file || file.error) {
        return response.status(401).send({ error: "internal server error" })
